"""
Extract and analyze both (FG + stereo) differences from pairs_fg_stereo_merged_nodot.csv

This script creates CSV files showing:
- toxic_smiles, nontoxic_smiles
- Full feature columns for each molecule (both FG and stereo)
- Difference columns
...
def create_both_diff_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Create a DataFrame with both FG and stereo difference information."""
    rows = []
    ...
    for idx, row in df.iterrows():
        ...
        # Full feature columns - Stereo (toxic)
        ...
        # Full feature columns - Stereo (nontoxic)
...
def main():
    """Main function to process both (FG + stereo) differences."""
    ...
    for step_type in ['one_step', 'multi_step']:
        ...
        diff_df = create_both_diff_dataframe(df_step)
        
        # Save to CSV
        output_file = output_dir / f'both_{step_type}_diff.csv'
