...
# -*- coding: utf-8 -*-
"""
`safe_qa_outputs_image/test` 아래 evaluation_summary_*.json 을 읽어서
Task1 / Task2 / Task3 및 Task3 Step-wise CoT 성능표를 하나의 CSV로 저장합니다.

기본 동작은 `ace_safe_ver/QA/LLMs/safe_qa_outputs/test/get_csv.py`와 동일하고,
이미지 포함 결과는 `safe_qa_outputs_image/` 아래에 저장되므로 base_dir만 분리해서 처리합니다.
"""
...
# 스크립트 위치: .../safe_qa_outputs_image/get_csv_image.py
_SCRIPT_DIR = Path(__file__).resolve().parent
_DEFAULT_BASE = _SCRIPT_DIR / "test"
...
# 고정 GPT 행 (inference_gpt.py 출력 파일명 슬러그와 일치)
...
def _extra_provider_rows(all_slugs: Set[str]) -> List[RowSpec]:
    """허용된 Gemini 슬러그만 3행(Both/SMILES/SAFE) 생성."""
    extra_slugs = sorted(
        s
        for s in all_slugs
...
def _row_task3_stepwise_cot_safe_final(data: Optional[Dict[str, Any]]) -> List[str]:
    """task3_stepwise_cot_safe_generation: Step3 = full SAFE + 메트릭."""
    m = _metrics_mean(data)
    return [
        _fmt(m.get("safe_EM")),
        _fmt(m.get("exact_match")),
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="safe_qa_outputs_image/test (task1/, task2/, task3/ 하위 포함)",
    )
    ap.add_argument(
        "-o",
        "--output",
    ...
    with open(out, "w", encoding="utf-8-sig", newline="") as f:
        ...
        _write_task_block(
            ...
            ["Step", "Model", "Mol Represent", "EM", "BLEU1", "Precision", "Recall", "F1", "Molecule EM", "Morgan FTS", "Validity"],
            _row_task2,
            base,
            "task2",
            tpl_task2,
        ...
        _write_task_block(
            ...
            ["Step", "Model", "Mol Represent", "EM", "BLEU1", "Levenshtein", "RDK FTS", "MACCS FTS", "Morgan FTS", "Validity"],
            _row_task3,
            base,
            "task3",
            tpl_task3,
        ...
        _write_task_block(
            ...
            ["Step", "Model", "Mol Represent", "SAFE EM", "SMILES EM", "BLEU1", "Levenshtein", "RDK FTS", "MACCS FTS", "Morgan FTS", "Validity"],
