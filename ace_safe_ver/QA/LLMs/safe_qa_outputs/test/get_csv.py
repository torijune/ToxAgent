...
행 순서:
  - 고정: GPT-4o×3, GPT-5.2×3, GPT-5 Both (각 Single/Multi Step)
  - 자동: 디렉터리에서 발견된 evaluation_summary_*.json 파일명 기준으로
    gemini-*, claude-* 슬러그마다 Both / Only SMILES / Only SAFE 3행 추가 (알파벳 순)
"""
...
_ROW_TEMPLATE_GPT: Sequence[RowSpec] = (
    ...
    ("GPT-5", "Both", "both_repre", ("gpt-5", "gpt-5.0")),
)

# CoT 블록: GPT-5 행 없음
_ROW_TEMPLATE_GPT_COT: Sequence[RowSpec] = tuple(_ROW_TEMPLATE_GPT[:6])
...
def _extra_provider_rows(all_slugs: Set[str]) -> List[RowSpec]:
    """gemini-*, claude-* 슬러그마다 3행(Both/SMILES/SAFE) 생성. 파일명 슬러그를 Model 열에 그대로 사용."""
    extra_slugs = sorted(
        s
        for s in all_slugs
        if s not in _STATIC_SLUGS and (s.lower().startswith("gemini") or s.lower().startswith("claude"))
    )
    rows: List[RowSpec] = []
