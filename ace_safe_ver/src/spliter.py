...
# NOTE: deepchem는 환경에 따라 tensorflow 등 의존성 때문에 import가 실패할 수 있습니다.
# (예: python 3.14에서는 tensorflow wheel 미지원) 이 파일의 일부 splitter(예: ScaffoldSplitter)는
# deepchem 없이도 동작 가능하므로, deepchem import를 optional로 둡니다.
try:
    import deepchem as dc  # type: ignore
    from deepchem.data import Dataset, DiskDataset  # type: ignore
    from deepchem.utils import get_print_threshold  # type: ignore
    ...
    if TYPE_CHECKING:  # pragma: no cover
        ...
        DiskDataset = Any  # type: ignore

        def get_print_threshold() -> int:  # type: ignore
            return 5
...
class UMAPSplitter(Splitter):
    """Class for doing data splits based on a UMAP projection of Morgan fingerprints.

    Molecules are first converted to Morgan bit fingerprints, projected into a
    low-dimensional UMAP space, then grouped with KMeans in that projected space.
    The resulting clusters are greedily assigned to train/valid/test splits.

    This splitter is useful when you want a chemical-space-aware split that is
    more flexible than a Bemis-Murcko scaffold split.

    Note
    ----
    This class requires RDKit, umap-learn, and scikit-learn to be installed.
    """
    ...
    def __init__(self,
        ...
        n_components: int, default 2
            Output dimensionality of the UMAP embedding.
        metric: str, default "jaccard"
            Distance metric used by UMAP.
        n_clusters: Optional[int], default None
            Number of KMeans clusters in UMAP space. If None, a heuristic based on
            dataset size is used.
        """
        super(UMAPSplitter, self).__init__()
    ...
    def split(
        ...
        """Split compounds into train/validation/test using UMAP-space clustering.

        Parameters
        ----------
        dataset: Dataset
            Dataset to be split. Molecule SMILES must be stored in `dataset.ids`.
        frac_train: float, default 0.8
            Fraction assigned to training split.
        frac_valid: float, default 0.1
            Fraction assigned to validation split.
        frac_test: float, default 0.1
            Fraction assigned to test split.
        seed: Optional[int], default None
            Random seed used for UMAP and KMeans.
        log_every_n: Optional[int], default None
        ...
        try:
            ...
            raise ImportError(
                "UMAPSplitter requires RDKit, umap-learn, and scikit-learn to be installed."
            ) from exc
        ...
        if self.n_clusters is None:
            n_clusters = max(3, int(np.sqrt(len(valid_indices))))
        else:
        ...
        if n_clusters <= 1:
            ...
            train_cutoff = int(frac_train * len(all_indices))
            valid_cutoff = int((frac_train + frac_valid) * len(all_indices))
            return (
                all_indices[:train_cutoff],
                all_indices[train_cutoff:valid_cutoff],
                all_indices[valid_cutoff:],
            )
        ...
        for original_idx, label in zip(valid_indices, labels):
            clusters.setdefault(int(label), []).append(original_idx)

        cluster_sets = sorted(clusters.values(), key=lambda x: (-len(x), x[0]))

        n_total = len(valid_indices)
        train_target = frac_train * n_total
        valid_target = frac_valid * n_total
        test_target = frac_test * n_total
        ...
        for cluster in cluster_sets:
            train_gap = train_target - len(train_inds)
            valid_gap = valid_target - len(valid_inds)
            test_gap = test_target - len(test_inds)

            if train_gap >= valid_gap and train_gap >= test_gap:
                train_inds.extend(cluster)
            elif valid_gap >= test_gap:
                valid_inds.extend(cluster)
            else:
                test_inds.extend(cluster)

        assigned = set(train_inds) | set(valid_inds) | set(test_inds)
        missing = [idx for idx in valid_indices if idx not in assigned]
        train_inds.extend(missing)

        return sorted(train_inds), sorted(valid_inds), sorted(test_inds)
...
class ScaffoldSplitter(Splitter):
    ...
    def split(
        ...
        if frac_valid == 0:
            # valid 없이 train/test만: test를 먼저 채우면(train 나중에) 큰 scaffold가 train으로 가서 train > test 유지
            # generate_scaffolds는 큰 것부터 정렬 → 뒤집어서 작은 것부터 순회
            logger.info("About to sort in scaffold sets (test-first for no valid)")
            ...
            # 기존: train → valid → test 순으로 채움 (큰 scaffold부터)
            logger.info("About to sort in scaffold sets")
            for scaffold_set in scaffold_sets:
                if len(train_inds) + len(scaffold_set) > train_cutoff:
                    if len(train_inds) + len(valid_inds) + len(
