"""Generate stereo_repair_gen QA set (only_stereo / one_step)"""
...
def _ez_bonds_safe(ez_raw: list) -> list:
    ez = []
    for b in ez_raw:
...
def _build_stereo_from_columns(row) -> dict:
    """Build toxic_stereochemistry dict. Converts bond lists to tuples."""
    chiral = parse_fg_list(row.get('toxic_chiral_centers', '[]'))
    ez = _ez_bonds_safe(parse_fg_list(row.get('toxic_ez_bonds', '[]')))
    return {
...
def _build_nontoxic_stereo_from_columns(row) -> dict:
    """Build nontoxic_stereochemistry dict. Converts bond lists to tuples."""
    chiral = parse_fg_list(row.get('nontoxic_chiral_centers', '[]'))
    ez = _ez_bonds_safe(parse_fg_list(row.get('nontoxic_ez_bonds', '[]')))
    return {
        'chiral_centers': {'chiral_center_count': len(chiral), 'chiral_centers': chiral},
        'ez_bonds': {'ez_bond_count': len(ez), 'ez_bonds': ez},
        'has_stereochemistry': bool(chiral or ez)
...
def generate_stereo_repair_gen_qa(
    csv_path: str,
    output_path: Optional[str] = None,
    smiles_format: str = "smiles",
    limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """Generate stereo_repair_gen QA items from only_stereo_one_step_diff.csv."""
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            # Try pre-stored stereochemistry first, fall back to building from individual columns
            ...
            question = template._create_stereo_repair_question(
            ...
            answer = template._create_stereo_repair_answer(nt_mol)

            qa_id = f"stereo_repair_gen_one_step_{idx}"
            context = create_base_context(row, "stereo_repair_gen", tx_smiles, nt_smiles)
            context.update({
                "toxic_stereochemistry": toxic_stereo,
                "nontoxic_stereochemistry": nontoxic_stereo,
                "stereochemistry_difference": stereo_diff
...
if __name__ == "__main__":
    ...
    qa_items = generate_stereo_repair_gen_qa(
