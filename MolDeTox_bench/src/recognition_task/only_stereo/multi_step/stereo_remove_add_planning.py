"""Generate stereo_repair_planning QA set (only_stereo / multi_step)"""
...
def generate_stereo_repair_planning_qa(
    csv_path: str,
    output_path: Optional[str] = None,
    smiles_format: str = "smiles",
    limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """Generate stereo_repair_planning QA items from only_stereo_multi_step_diff.csv."""
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            question = template._create_stereo_repair_planning_question(
                tx_mol=tx_mol, mol_format=mol_format, stereo_summary=stereo_summary,
            ...
            answer = template._create_stereo_repair_planning_answer(comparison)

            qa_id = f"stereo_repair_planning_multi_step_{idx}"
            context = create_base_context(row, "stereo_repair_planning", tx_smiles, nt_smiles)
            context.update({
                "toxic_stereochemistry": toxic_stereo,
                "nontoxic_stereochemistry": nontoxic_stereo,
                "stereochemistry_difference": stereo_diff
...
if __name__ == "__main__":
    ...
    qa_items = generate_stereo_repair_planning_qa(
