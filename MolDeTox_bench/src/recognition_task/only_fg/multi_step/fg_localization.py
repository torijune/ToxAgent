"""Generate toxic_site_localization QA set (only_fg / multi_step)"""
...
def generate_toxic_site_localization_qa(
    csv_path: str,
    output_path: Optional[str] = None,
    smiles_format: str = "smiles",
    limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """Generate toxic_site_localization QA items from only_fg_multi_step_diff.csv."""
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            tx_mol_with_atoms = add_atom_numbers_to_smiles(tx_mol) if smiles_format == "smiles" else tx_mol
            ...
            question = template.create_question(
                nt_fgs=nt_fgs, tx_fgs=tx_fgs, nt_mol="",
                tx_mol=tx_mol_with_atoms, mol_format=mol_format,
                question_type="toxic_site_localization",
            ...
            answer = template.create_answer(
                ...
                question_type="toxic_site_localization", subset="TN",
                tx_fg_full=tx_fg_full
            ,
                step_type="multi_step"
            )

            qa_id = f"toxic_site_localization_multi_step_{idx}"
            context = create_base_context(row, "toxic_site_localization", tx_smiles, nt_smiles)
...
if __name__ == "__main__":
    ...
    qa_items = generate_toxic_site_localization_qa(
