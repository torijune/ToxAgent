"""Generate fg_identification QA set (only_fg / one_step)"""
...
def generate_fg_identification_qa(
    csv_path: str,
    output_path: Optional[str] = None,
    smiles_format: str = "smiles",
    limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """Generate fg_identification QA items from only_fg_one_step_diff.csv."""
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            common_fgs = sorted(set(tx_fgs) & set(nt_fgs))
            ...
            question = template.create_question(
                nt_fgs=nt_fgs, tx_fgs=tx_fgs, nt_mol=nt_mol, tx_mol=tx_mol,
                mol_format=mol_format, question_type="fg_identification",
                common_fgs=common_fgs, correct_fg=tx_unique[0] if tx_unique else None,
                nt_unique=nt_unique, tx_fg_full=tx_fg_full, nt_fg_full=nt_fg_full,
            ...
            answer = template.create_answer(
                ...
                question_type="fg_identification", subset="TN", common_fgs=common_fgs
            ,
                step_type="one_step"
            )

            qa_id = f"fg_identification_one_step_{idx}"
            context = create_base_context(row, "fg_identification", tx_smiles, nt_smiles)
